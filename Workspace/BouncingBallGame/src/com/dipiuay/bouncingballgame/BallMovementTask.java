...
public class BallMovementTask extends TimerTask {
	...
	private void moveStraight(float distance){
		...
		float hitDistance = ball.radius + target.radius;
		float targetProjection = Algebra.dotProduct(Algebra.substract(target.position, startPosition), direction);
		if(0 <= targetProjection && targetProjection <= distance){
			PointF projectionPoint = Algebra.add(startPosition, Algebra.multiply(targetProjection, direction));
			float projectionDistance = Algebra.getLength(Algebra.substract(projectionPoint, target.position));			
			if(projectionDistance <= hitDistance){
				isTargetHit = true;
			}
		}
		else if(Algebra.getLength(Algebra.substract(target.position, startPosition)) <= hitDistance){
			isTargetHit = true;
		}
		else if(Algebra.getLength(Algebra.substract(target.position, endPosition)) <= hitDistance){
			isTargetHit = true;
		}
		
		if(isTargetHit){
			this.game.score += Game.scoreDelta;
			this.game.moveTargetRandomly();
		}
	}
